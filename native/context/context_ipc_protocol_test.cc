...
int main() {
  ...
  // Focus-switch regression: a fresh capability may restart revision at 1,
  // but the retired capability must never become active again.
  ContextFrame next_focus = ValidFrame(1);
  next_focus.source_capability[0] ^= 0x80U;
  std::vector<std::uint8_t> next_encoded;
  assert(EncodeContextFrame(next_focus, &next_encoded));
  assert(receiver.Accept(Bytes(next_encoded)) ==
         ContextFrameAcceptResult::kAccepted);

  ContextFrame resurrected = ValidFrame(8);
  std::vector<std::uint8_t> resurrected_encoded;
  assert(EncodeContextFrame(resurrected, &resurrected_encoded));
  assert(receiver.Accept(Bytes(resurrected_encoded)) ==
         ContextFrameAcceptResult::kStale);
