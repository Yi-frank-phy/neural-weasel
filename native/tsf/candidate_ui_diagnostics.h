...
inline void WriteCompositionDiagnostic(const char* event,
                                       bool tsf_composition_active,
                                       bool rime_composing,
                                       const void* current_composition,
                                       const void* callback_composition) {
  if (event == nullptr) {
    return;
  }
  ...
  const int line_size = sprintf_s(
      line, sizeof(line),
      "tick=%llu pid=%lu tid=%lu event=composition-%s "
      "tsf_composition_active=%d rime_composing=%d current_composition=%p "
      "callback_composition=%p same_composition=%d\r\n",
      static_cast<unsigned long long>(GetTickCount64()),
      static_cast<unsigned long>(GetCurrentProcessId()),
      static_cast<unsigned long>(GetCurrentThreadId()), event,
      tsf_composition_active ? 1 : 0, rime_composing ? 1 : 0,
      current_composition, callback_composition,
      current_composition != nullptr && current_composition == callback_composition ? 1
                                                                                     : 0);
  if (line_size > 0) {
    DWORD written = 0;
    WriteFile(file, line, static_cast<DWORD>(line_size), &written, nullptr);
