...
if (-not (Get-Command Replace-Literal -ErrorAction SilentlyContinue)) {
    throw 'Presentation overlay must run after prepare-weasel-overlay.ps1 in the same PowerShell scope.'
}
if (-not (Get-Variable ResolvedWeaselRoot -ErrorAction SilentlyContinue)) {
    throw 'Pinned Weasel root was not established by the base overlay.'
}

# ProcessKeyEvent's boolean means "handled", not "presentation ready". Carry
# one identity-free readiness bit through Weasel's existing status response so
# the private U+FDD0 refresh key stays handled and never enters commit history.
