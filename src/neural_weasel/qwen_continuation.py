...
class QwenContinuationSession:
    ...
    def advance(self, parent_indices: Sequence[int], token_ids: Sequence[int]) -> float:
        ...
        with self.runtime._lock, torch.inference_mode():
            ...
            input_ids = torch.as_tensor(tokens, dtype=torch.long, device="cuda:0").reshape(-1, 1)
            next_length = self.total_length + 1
            attention_mask = torch.ones((len(tokens), next_length), dtype=torch.long, device="cuda:0")
            ...
            self.log_probs = torch.log_softmax(outputs.logits[:, -1, :].float(), dim=-1).detach()
            return elapsed_ms

    def score_allowed(self, allowed_by_beam: Sequence[Sequence[int]]) -> tuple[np.ndarray, ...]:
        ...
        for row, allowed in enumerate(allowed_by_beam):
            ...
            if not ids:
                ...
                continue
            index = torch.as_tensor(ids, dtype=torch.long, device=self.log_probs.device)
