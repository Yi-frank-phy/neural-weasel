...
SCHEMA = ROOT / "assets" / "rime" / "neural_weasel.schema.yaml"
KEY_PROCESSOR = ROOT / "native" / "rime" / "bilingual_key_processor.cc"
AI_TRANSLATOR = ROOT / "native" / "rime" / "ai_translator.cc"
...
def test_number_keys_reach_selector_before_speller() -> None:
    schema = yaml.safe_load(SCHEMA.read_text(encoding="utf-8"))
    processors = schema["engine"]["processors"]

    assert processors.index("bilingual_key_processor") < processors.index("selector")
...
def test_english_number_keys_are_reinserted_as_literal_input() -> None:
    ...
    assert "intent == KeyIntent::kNumberedSelection" in source
    assert "context->PushInput(static_cast<char>(key_event.keycode()))" in source


def test_internal_coverage_marker_is_not_rendered_in_candidate_comments() -> None:
    source = AI_TRANSLATOR.read_text(encoding="utf-8")

    assert 'comment += "[coverage]"' not in source
